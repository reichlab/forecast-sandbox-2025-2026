...
model_out_tbl <- dplyr::collect(hub_con) |>
  ...
  hubEnsembles::simple_ensemble(model_id = "UMass-flusion_spatial")
...
output_dir <- "../../model-output/UMass-flusion_spatial"
...
utils::write.csv(
  ...
  file = file.path(
    output_dir,
    paste0(reference_date, "-UMass-flusion_spatial.csv")
  ),
  row.names = FALSE
)
