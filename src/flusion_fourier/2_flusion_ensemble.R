...
model_out_tbl <- dplyr::collect(hub_con) |>
  ...
  hubEnsembles::simple_ensemble(model_id = "UMass-flusion_fourier")
...
output_dir <- "../../model-output/UMass-flusion_fourier"
...
utils::write.csv(
  ...
  file = file.path(
    output_dir,
    paste0(reference_date, "-UMass-flusion_fourier.csv")
  ),
  row.names = FALSE
)
