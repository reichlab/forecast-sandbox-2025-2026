"""
Diagnostic version of gbqr_3src_spatial main.py with artifact saving enabled.
Run this to save feature importance and model artifacts for analysis.
"""
import click
import datetime
...
def main(today_date: str, short_run: bool, artifact_dir: str):
    """Generate flu predictions from gbqr spatial model with diagnostic outputs."""
    try:
    ...
    model_config = SimpleNamespace(
        ...
        # directional wave features for spatial modeling
    ...
    run_config = SimpleNamespace(
        ...
        artifact_store_root=Path(artifact_dir) / reference_date.isoformat(),
        save_feat_importance=True,  # ENABLED for diagnostics
        max_horizon=4,
        locations=["01", "02", "04", "05", "06", "08", "09", "10", "11",
    ...
    print(f"Running diagnostic forecast for {reference_date}")
    print(f"Artifacts will be saved to: {run_config.artifact_store_root}")

    model = GBQRModel(model_config)
    model.run(run_config)

    print(f"Diagnostic run complete. Check {run_config.artifact_store_root} for outputs.")
