...
# load all three components and create ensemble
model_out_tbl <- dplyr::collect(hub_con) |>
  dplyr::filter(reference_date == ref_date, horizon >= 0) |>
  hubEnsembles::simple_ensemble(model_id = "UMass-flusion3")
...
output_dir <- "../../model-output/UMass-flusion3"
...
utils::write.csv(
  ...
  file = file.path(
    output_dir,
    paste0(reference_date, "-UMass-flusion3.csv")
  ),
  row.names = FALSE
)
